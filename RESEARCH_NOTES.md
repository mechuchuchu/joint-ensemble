...
successive correction networks reduce residual training error.
...
- Correction network: `64 -> 16 -> 16 -> 10`, with ReLU activations
- Optimizer: AdamW, learning rate `1e-2`, zero weight decay
- Three trials per vector count and eight correction stages
- 100-epoch budget per stage with the same vector-scaled step budget used by
  the random-vector memorization experiment
- Metrics measured on the complete memorization set after every epoch
- No comparison conditions

One correction network has 1,482 parameters. Eight stages therefore contain
11,856 cumulative parameters, although only one stage is trainable at a time.

The mean training accuracy across three trials was:
...
For 8,192 vectors, accuracy increased monotonically from approximately 23% to
81%, while mean NLL decreased from 2.056 at stage 1 to 0.628 at stage 8. The
epoch-wise curves show each new stage starting from the preceding solution and
learning an additional residual improvement.

![Epoch-wise additive correction curves](additive-logit-correction-results/epoch_curves.png)

These results are consistent with residual correction in logit space: a new
network can focus on remaining errors instead of recreating the entire
classifier. This is not a useful-data generalization claim. Random-label
memorization probes optimization and cumulative fitting capacity, and does not
establish communication or wall-clock efficiency in a distributed system.

Artifacts are stored in `additive_logit_correction_memorization.py` and
`additive-logit-correction-results/`, including stage-level JSON/CSV records,
epoch-level metrics, and the generated figure.
